...
//Function to spawn a person on a floor
void Building::spawnPerson(Person newPerson){

    floors[newPerson.getCurrentFloor()].
           addPerson(newPerson,newPerson.getTargetFloor() 
         - newPerson.getCurrentFloor());
}

//Function to process service requests and designate pickup moves
void Building::update(Move move){

    if (!move.isPassMove()) {
        elevators[move.getElevatorId()].
                  serviceRequest(move.getTargetFloor());
        ...
        if (move.isPickupMove()) {
            ...
            floors[elevators[move.getElevatorId()].getCurrentFloor()].
                             removePeople(peopleToBePickedUp, 
                             move.getNumPeopleToPickup());
        }
    }
}

//Function to determine tick speed and time that each move takes
